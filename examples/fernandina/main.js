...
function initialization(index, zCavity, zGrid) {
    ...
    function save() {
        ...
        const simulation_dikes_filename = path + params.grid.out.path + `simulations-dykes-${index}.xyz`
    ...
    const params = JSON.parse( fs.readFileSync('Fernandina.json', 'utf8') ).initialization
    ...
    params.cavity.filename.forEach( cavityName => {
        const s = io.decodeGocadTS( fs.readFileSync(path + pathC + cavityName, 'utf8'), {repair: true} )
        if (s===undefined || s.length===0) {
            console.error(`cavities ${path + pathC + cavityName} cannot be loaded`) ;
            exit(-1)
        }
        s.forEach (S => {
            cavities.push(S)
    ...
    model.setMaterial ( params.model.poisson, params.model.young, params.model.rockDensity )
    ...
    cavities.forEach( (surf,i) => {
        ...
        surf.series.positions = df.apply(surf.series.positions, p => [p[0]+t[0], p[1]+t[1], p[2]+t[2]+zCavity])
    ...
    if (params.model.check) {
        ...
        if (params.model.halfSpace) {
            cavities.forEach( (surf, i) => {
                ...
                if (mm[5]>0) {
                    console.log('ERROR: Model in half-space and disconinuity at index '+i+' have some z > 0.')
                    exit(-1)
                }
            })
        }
    ...
    params.grid.filename.forEach( gridName => {
        ...
        if (gridName.endsWith('xyz')) {
            const allgrids = io.decodeXYZ( fs.readFileSync(path + pathG + gridName, 'utf8') )
            grids = [...grids, ...allgrids]
        }
        else if (gridName.endsWith('ts') || gridName.endsWith('gcd')) {
            const allgrids = io.decodeGocadTS( fs.readFileSync(path + pathG + gridName, 'utf8') )
            grids = [...grids, ...allgrids]
        }
        else {
            console.error('Unknown file format for grid: ' + path + pathG + gridName) ;
            exit(-1)
        }
    })
    ...
    grids.forEach( (grid, i) => {
        console.log('grid '+(i+1)+' nb-points:', grid.series['positions'].count)
    ...
    // translate grids ?
    {
        ...
        grids.forEach( grid => grid.series.positions = df.apply( grid.series.positions, p => [p[0]+t[0], p[1]+t[1], p[2]+t[2]+zGrid] ) )
    }
    ...
    if (params.model.check && params.model.halfSpace) {
        grids.forEach( (grid, i) => {
            ...
            if (mm[5]>0) {
                console.log('ERROR: Model in half-space and grid at index '+i+' have some z > 0.')
                exit(-1)
            }
        })
    }
        ...
        */
        ...
        if (params.model.perCavity === true) {
            chambers.forEach( (chamber, i) => {
                if (useGravity) {
                    chamber.setBC( "normal", "free", (x,y,z) => alpha[4]*Math.abs(z)*9.81 + alpha[5+i] )
            ...
            chambers.forEach( chamber => {
                if (useGravity) {
                    chamber.setBC( "normal", "free", (x,y,z) => alpha[4]*Math.abs(z)*9.81 + alpha[5] )
        ...
        if (params.cavity.out.burgers.active) {
            ...
            cavities.forEach( (surf, i) => {
                surf.series[`${params.cavity.out.burgers.name}${index}`] = df.Serie.create({array: burgers[i] , itemSize: 3})
            })
            if (params.cavity.info) {
                burgers.forEach( (burger, i) => {
                    console.log('  - Burger min-max for surface ' + i + ': ' + math.minMaxArray(burger) )
                })
            }
        }
        ...
        solution.onProgress( (i,p) => console.log(`nb-pts so far: ${i}, realized: ${p.toFixed(0)}%`) )
        ...
        grids.forEach( (grid, i) => {
            ...
            if (params.grid.out.displ.active)  {
                ...
                if (params.grid.info) {
                    console.log('  - Displ min-max for grid ' + i + ': ' + math.minMaxArray(displ.array) )
                }
            }
            if (params.grid.out.strain.active) {
                const strain = df.Serie.create({array: solution.strain(obs), itemSize: 6})
                grid.series[`${params.grid.out.strain.name}${index}`] = strain
                if (params.grid.info) {
                    console.log('  - Strain min-max for grid ' + i + ': ' + math.minMaxArray(strain.array) )
                }
            }
            if (params.grid.out.stress.active) {
                ...
                if (params.grid.info) {
                    console.log('  - Stress min-max for grid ' + i + ': ' + math.minMaxArray(stress.array) )
                }
            }
        })
    }
    ...
    console.info('Execution time for superposition of '+nbSimulations+': %dms', end)
...
function invert(index, zCavity, zGrid) {
    ...
    const paramsModel = JSON.parse( fs.readFileSync('Fernandina.json', 'utf8') ).initialization
    const params = JSON.parse( fs.readFileSync('Fernandina.json', 'utf8') ).inversion
    const simulation_dikes_filename = paramsModel.model.path + paramsModel.grid.out.path + `simulations-dykes-${index}.xyz`
    ...
    if (params.inverse && params.inverse.active) {
        ...
        dataframe.series['newN'] = df.apply(data.generate(alpha), n => [-n[1], n[0], 0] )
        ...
        dataframe.series['misfit'] = data.costs(alpha).map( v => v*180/Math.PI )
        ...
        console.log('writting file:', params.model.path + params.inverse.out.path + params.inverse.out.filename)
        fs.writeFileSync(params.model.path + params.inverse.out.path + `result-forward-dikes-${index}.xyz`, bufferOut, 'utf8', err => {})
    }
    else {
    ...
    if (params.forward !== undefined && params.forward.active) {
        ...
        // Discontinuity
        const surfs = io.decodeGocadTS( fs.readFileSync(params.model.path + params.forward.cavity.path + params.forward.cavity.filename, 'utf8'), {repair: false} )
        const chambers = []
        ...
        if (alpha.length === 5+surfs.length) {
            ...
            if (alpha.length !== 6) {
                console.error('Someting is wrong: alpha size is strange. Getting', alpha.length)
                exit(-1)
            }
        }
        ...
        const grid = geom.generateRectangle({a: minMax[3]-minMax[0], b: minMax[4]-minMax[1], na: 50, nb: 50, center: [(minMax[3]+minMax[0])/2, (minMax[4]+minMax[1])/2, 0]})
        ...
        grid.series.positions = df.apply(grid.series.positions, p => [p[0]+t[0], p[1]+t[1], p[2]+t[2]+zGrid])
        const obs  = grid.series['positions'].array
        ...
        grid.series['Joint'] = geo.generateJoints({stress: S, projected: true}).map( v => [v[1], -v[0], 0] )
        grid.series['S'] = S
    
        // -----------------------------------------------
        fs.writeFileSync(params.model.path + params.forward.grid.out.path + `result-grid-${index}.ts`, io.encodeGocadTS(grid, {
            expandAttributes: false,
            userData: {
                result: params.forward.alpha ? params.forward.alpha : JSON.stringify(result)
            }
        }), 'utf8')
        // -----------------------------------------------
        grid.remove('indices')
        fs.writeFileSync(params.model.path + params.forward.grid.out.path + `result-grid-${index}.xyz`, io.encodeXYZ(grid, {
            expandAttributes: true,
            userData: {
                result: params.forward.alpha ? params.forward.alpha : JSON.stringify(result)
            }
