...
const chambers = []
chambers.push(
    addDiscon('/Users/fmaerten/data/arch/figure-superposition/s1.ts', model),
)
chambers.push(
    addDiscon(
        '/Users/fmaerten/data/arch/figure-superposition/chamber-modif.ts',
        model,
    ),
)
...
const doSimulation = (index) => {
    ...
    chambers.forEach((c) =>
        c.setBC(
            'normal',
            'free',
            (x, y, z) => alpha[6] * params.g * Math.abs(z) + alpha[7],
        ),
    )
