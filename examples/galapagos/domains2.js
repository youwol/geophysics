...
if (1) {
    ...
    for (let i=0; i<N; ++i) {
        ...
        surface.series['cost'] = positions.map( p => {
            ...
            userAlpha[4] = densityMin + i*(densityMax-densityMin)/(N-1)
        ...
        fs.writeFileSync(path + `/domain-${i}.ts`, io.encodeGocadTS(surface), 'utf8', err => {})
        console.log('')
    }
}
...
if (0) {
    ...
    if (0) {
        ...
        surface = geom.generateEllipse({a:1, b:1, nbRings:30, density:8, center:[0, 0, 0]})
    }
    ...
    fs.writeFileSync(path + `domain-theta-Rh-0.ts`, io.encodeGocadTS(surface), 'utf8', err => {} )
}
