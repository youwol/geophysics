...
const grids = io.decodeGocadTS( fs.readFileSync(path+'/'+'forward-grid.ts', 'utf8') )
...
function rotateAxis(serie, AXIS, angleInDeg) {
    if (serie.itemSize === 3) {
        ...
        return serie.map( m => {
            return packSMatrix3( math.mat.rotate(unpackSMatrix3(m), angleInDeg, AXIS) )
        })
    }
    return undefined
}
...
fs.writeFileSync(path+'/forward-grid-rotated.ts', io.encodeGocadTS(grids, {
