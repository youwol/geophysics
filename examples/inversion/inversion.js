...
function generateData(args, data, dataframe) {
    if (data['attribute'] === undefined) {
        ...
        data["compute"] = new Array(args.dim).fill(0).map((_v, i) => {
            ...
            if (dataframe.contains(name) === false) {
                throw new Error(`Serie '${name}' does not exist in data ${data.file}`)
            }
            return name
        })
    }
    ...
    switch (data.type) {
        case 'joint':
        case 'dike':
        case 'dyke': return new geop.JointData(data)
        case 'stylolite': return new geop.StyloliteData(data)
        case 'conjugate': return new geop.ConjugateData(data)
        case 'scalar': return new geop.GenericScalarFieldData(data)
        case 'gps': return new geop.GpsData(data)
        case 'vgps': return new geop.VerticalGpsData(data)
        default: {
            throw new Error(`Unknown data type "${data.type}". Should be "joint", "dyke", "dike", "stylolite", "conjugate", "scalar, "gps" or "vgps" (for the moment)`)
        }
    }
}
...
function displaySolution(_args, solution, mapping) {
    ...
    console.log('iteration   ', solution.iteration + '/' + solution.maxIteration)
...
function postprocess(_args, grids, alpha) {
    ...
    grids.in.forEach((inFilename, index) => {
        ...
        if (filter === undefined) {
            ...
            const dataframes = filter.decode(buffer, { shared: false, merge: true })
            ...
            fs.writeFileSync(outFilename, filter.encode(dfs, {
...
function postprocessOnData(datas, solution) {
    ...
    datas.forEach((data) => {
        ...
        const prefix = data.data.out.prefix !== undefined ? data.data.out.prefix : 'forward'
    ...
    // Now save the data
    d.forEach((dd) => { // dd is an array of Data
        if (dd.length === 0) {
            return
        }
        ...
        if (dd[0].data.out.file) {
            ...
            if (filter === undefined) {
                ...
                fs.writeFileSync(filename, filter.encode(dfs, {
                    expandAttributes: true
                }), 'utf8', err => { })
            }
        }
...
function doDomain(args, params, solution) {
    if (args.postprocess.domain !== undefined && args.postprocess.domain.active === true) {
        ...
        const domain = d.evaluate(xAxisIndex, yAxisIndex, args.postprocess.domain.user !== undefined ? args.postprocess.domain.user : solution.user)

        const array = []
        let k = 0
        const flatten = args.postprocess.domain.flatten !== undefined ? args.postprocess.domain.flatten : false
        ...
        fs.writeFile(filename, filter.encode(T, {
            userData: {
                userAlpha: args.postprocess.domain.user !== undefined ? args.postprocess.domain.user : solution.user,
...
args.datas.forEach((DATA, dataId) => {
    if (DATA.active === undefined || (DATA.active !== undefined && DATA.active === true)) {
        ...
        if (filter === undefined) {
            ...
            const dataframes = filter.decode(buffer, { shared: false, merge: true }) // !!! BEFORE: Take only the first loaded dataframe
...
if (mapping === undefined) {
    throw new Error(`Unknown mapping function ${args.mapping}. Possible values are ${geop.MappingFactory.names().map(n => ' '+n)}`)
}
...
// DOMAIN
// ===========================================
doDomain(args, params, solution, args.postprocess.domain.xAxis, args.postprocess.domain.yAxis)
