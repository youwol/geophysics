...
import { abs, div, dot, eigen, normalize, rotateAxis, square, sub, vec, weightedSum } from "@youwol/math"
import { Data } from "../data"
import { Alpha } from "../types"

const getTheta = (friction: number): number => Math.PI*(45 - friction/2)/180
...
export class ConjugateData extends Data {
    ...
    constructor(
        {dataframe, measure, compute, weights, weight, friction=0.3, projected=false}:
        {dataframe: DataFrame, measure: string, compute?: string[], weights?: string, weight?: number, friction: number, projected?: boolean})
    {
    ...
    costs(alpha: Serie | Alpha): Serie {
        ...
        if (this.projected) {
            if (d.itemSize !== 3) throw new Error('generateData must have itemSize = 3 (i.e., normal)')
