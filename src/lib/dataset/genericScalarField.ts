...
import { weightedSum, PositionDecomposer, minMax, normalize } from '@youwol/math'
...
export class GenericScalarFieldData extends Data {
    ...
    constructor(
        {dataframe, positions, measure, coordIndex, compute, weights, weight=1, xScale=1, yScale=1, useDerivative=false}:
        {dataframe: DataFrame, positions: string, measure: string, coordIndex: number, compute?: string[], weights?: string, weight?: number, xScale?: number, yScale?: number, useDerivative?: boolean})
    {
    ...
    costs(data: Serie | Alpha): Serie {
        ...
        if (compute.itemSize !== 1) throw new Error('generateData must have itemSize = 1')
...
export function generateGenericScalarField(
    {data, coordIndex, positions, xScale, yScale, useDerivative}:
    {data: Serie, coordIndex: number, positions: Serie, xScale: number, yScale: number, useDerivative: boolean}): Serie
{
    if (useDerivative) {
        return derivative( {
...
//
function derivative(
    {data, positions, xScale, yScale}:
    {data: Serie, positions: Serie, xScale: number, yScale: number}): Serie
{
