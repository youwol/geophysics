...
import { 
    ...
    div, mult, sub, norm, negate, addNumber, mean
 ...
 * @category Geology
 */
export function costStriation(obs: ASerie, calc: ASerie, w = 1): number {
    const d  = dot(obs, calc)
    const no = norm(obs)
    const nc = norm(calc)
    return mean(mult( addNumber(negate(abs(div(dot(obs, calc), mult(no, nc)))), 1), w)) as number
 ...
 * @category Geology
 */
export function costJoint(n: ASerie, stress: ASerie, w: number = 1): number {
    const e  = eigenVector(stress).map( v => [v[0], v[1], v[2]] )
    const ns = normalize(n)
    return mean(mult(square(sub(abs(dot(ns, e)), 1)), w)) as number // w*(1-d)**2
}
 ...
 * @category Geology
 */
export function costStylolite(n: ASerie, stress: ASerie, w: number = 1): number {
    const e  = eigenVector(stress).map( v => [v[6], v[7], v[8]] )
    const ns = normalize(n)
    return mean(mult(square(sub(abs(dot(ns, e)), 1)), w)) as number
}
...
export function costConjugate(n: ASerie, stress: ASerie, w = 1, fric = 60, projected = false) {
    ...
    if (projected) {
        return mean(n.map( (n1: number[], i: number) => {
            const d  = Math.sqrt(n1[0]**2 + n1[1]**2)
            const nn = [n1[0]/d, n1[1]/d, 0]
            const N  = fractureNormal(stress.itemAt(i) as Vector6, projected) // Sigma2, so very simple
            return fractureCost(nn as Vector3, N as Vector3)
        }) ) as number
    }

    const theta  = Math.PI*(45.0 - fric/2.0)/180
    return mean(normalize(n).map( (normal, i) => {
        const conjugate = conjugateNormals(stress.itemAt(i) as Vector6, theta)
