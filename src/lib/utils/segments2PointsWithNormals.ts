 ...
 * ``` 
 */
export function segments2PointsWithNormals(df: DataFrame, dipAngle = 90, normalsName = 'n'): DataFrame {
    ...
    df.series.indices.forEach( P => {
        ...
        let v = vec.normalize( [(v2[1]-v1[1])*SIN, -(v2[0]-v1[0])*SIN, COS] )
        v = vec.normalize(v)
    ...
    dataframe.series[normalsName] = Serie.create({array: normals, itemSize: 3})
